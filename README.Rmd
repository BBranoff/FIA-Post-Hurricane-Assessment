...
tabs = "from fs_nims_fiadb_srs.plotsnap p,fs_nims_fiadb_srs.cond c,fs_nims_fiadb_srs.pop_stratum ps,fs_nims_fiadb_srs.pop_plot_stratum_assgn pps,fs_nims_fiadb_srs.POP_ESTN_UNIT PEU "
...
##  cacheing doesnt work on raster objects, because they are held in memory
...
FIA_units <- read_sf("//usda.net/fs/FS/RD/SRS/Science/FIA/Collaboration/SRS_GIS_Layers/SRS/srs_unit.shp")
...
Helene_Cat_plots <- trim(mask(Helene_Cat,st_transform(FIAstates,crs(Helene_Cat_plots))))#st_transform(st_as_sf(st_concave_hull(st_union(plots_geo),ratio=0.1)),crs(Helene_Cat))))
...
Areas <- rbind(data.frame(Areas12)|>mutate(STATECD=12),
  ...
  rename(npixels=Freq)|>
  mutate(area.raster.m = npixels*res(Helene_Cat_plots )[1]*res(Helene_Cat_plots )[2],
         area.raster.ac = area.raster.m/4046.86)|>
  group_by(STATECD)|>
  mutate(state_tot_pixels=sum(npixels))|>
  left_join(FIAstates|>
  filter(STATEFP %in% c(12,13))|>
    mutate(STATECD=as.numeric(STATEFP)) |>
  st_transform(st_crs(Helene_Cat_shape))%>%
    mutate(statearea.ac=as.numeric(st_area(.))/4046.86),by=join_by(STATECD))|>st_drop_geometry()|>ungroup()

### compare the two
Areas <- Areas |> left_join(Helene_Cat_shape|>
  ...
  mutate(AreaFIA.m=as.numeric(st_area(.)),
...
plots_geo<- plots_geo |>
  ...
  group_by(EVAL_GRP,EVALID,WindZone)|>
  mutate(nplots = length(unique(CN)),
         EXPNS_new_raster = (npixels/state_tot_pixels)*(statearea.ac/nplots))
...
ForestedAcres <- plots_geo |>
  ...
  group_by(EVAL_GRP)|>
  summarise(ForestedAcres_orig=sum(ForestedAcres*EXPNS),
            ForestedAcres_new=sum(ForestedAcres*EXPNS_new_raster))|>
...
#   summarise(ForestedAcres=sum(ForestedAcres*EXPNS_new_FIA))
  
HeleneAcres|>
  filter(EVAL_GRP%in% c(122024,132024))|>#,FORTYP=="All",County=="All")|>
  group_by(EVAL_GRP) |>
  summarise(ForestedAcres=sum(ForestedAcres_new,na.rm=TRUE))|>
